const WHEEE_GIF_SRC = require('./wheee_gif_base64.js');
const WHEEE_MP3_SRC = require('./wheee_mp3_base64.js');
...
exports.reduceUI = (state, action) => {
  switch (action.type) {
    case 'WHEEE_START':
      return state.set('wheeeMode', true);
    case 'WHEEE_END':
      return state.set('wheeeMode', false);
  }
  return state;
};
...
exports.mapTermsState = (state, map) => {
  return Object.assign(map, {
    wheeeMode: state.ui.wheeeMode
  });
};

const passProps = (uid, parentProps, props) => {
  return Object.assign(props, {
    wheeeMode: parentProps.wheeeMode
  });
}
...
exports.decorateTerm = (Term, { React, notify }) => {
  return class extends React.Component {
    ...
    _initDoItAudio () {
      this._audio = new Audio(WHEEE_MP3_SRC);
    ...
    _playDoIt() {
      ...
      // hide img
      setTimeout(function() { 
        this._img.style.right = this._window.innerWidth + 'px';
      }.bind(this), 100);
      setTimeout(function() { 
    ...
    _initDoItImage () {
      ...
      this._img.src = WHEEE_GIF_SRC;
      this._img.style.position = 'absolute';
      this._img.style.width = '60px';
      this._img.style.heigth = '60px';
      this._img.style.bottom = '-16px';
      this._img.style.right = '-60px';
      this._img.style.transitionProperty = 'all';
      this._img.style.transitionDuration = '5s';
    }
    ...
    _spawnParticles (x, y) {
      ...
      const colors = this.props.wheeeMode
    ...
    _shake (power = 1) {
      if(!this.props.wheeeMode) return;
    ...
    componentWillReceiveProps (next) {
      if (next.wheeeMode && !this.props.wheeeMode) {
        notify('DO IT !');
      } else if (!next.wheeeMode && this.props.wheeeMode) {
        notify('NO !!!!');
      }
    }
